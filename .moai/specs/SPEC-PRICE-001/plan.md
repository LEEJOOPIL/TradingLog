...
> **후속 업데이트 (2026-08-25, run-phase 완료 보고 이후) — M4 서버 구현은 클라이언트 구현으로 대체됨.** 위 M4 계획 내용은 수정하지 않고 그대로 둔다 — 이 문단은 사후 기록으로 추가하는 것이다. M4의 서버 함수(`fetchBinancePrice`, `tryBinanceHost_` 포함)는 위 계획대로 2026-08-23에 구현·커밋됐다(commit `f615e57`, 이후 지역 차단 대응 커밋 `3863368`/`c19d9f8`계열). 그러나 run-phase 완료 보고 이후 사용자의 실기 배포 테스트에서 Google 클라우드 서버 IP에 대한 지역·세션별 접근 차단(호스트별로 HTTP 451 → 403 순차 재현, 두 호스트 자동 재시도로도 해소되지 않음)이 발견되어, 2026-08-25에 바이낸스 조회 전체를 서버에서 클라이언트(`Index.html`, 네이티브 `fetch()`)로 이전하는 구조적 변경이 사용자 승인으로 이루어졌다(commit `f1d4774`). 그 결과 `Code.gs`의 `fetchBinancePrice`/`tryBinanceHost_`는 삭제됐고, 반올림 공식·0 붕괴 가드·오류 메시지 문구는 동일한 리터럴로 `Index.html`의 `fetchBinancePriceClient_`로 이식됐다(M5 절 참조 — M5가 호출하는 대상이 서버 RPC에서 이 클라이언트 함수로 바뀌었다). 근거·경위 전체(HTTP 상태 코드, 실행 컨텍스트별 재현 결과, 커밋 SHA 단위 증거)는 `progress.md`의 "후속 수정" 절(4건, 2026-08-25)에 시간순으로 기록되어 있다. 요구사항 수준 변경 근거는 `spec.md` §2.5, 위치 변경 근거는 `spec.md` §3.6.1 참조.
