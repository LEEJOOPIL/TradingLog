...
| 가격 소스 | 수동 입력(기본/전 자산) + 바이낸스 현물가 조회(보조, 심볼 등록 자산만) + 금·은 gold-api.com 현물시세 조회(보조, 금속시세 심볼 등록 자산만) | 조회는 브라우저 `fetch()`로 클라이언트에서 직접 수행 — Apps Script 서버(`UrlFetchApp`)를 거치지 않음. 구글 클라우드 서버 IP가 바이낸스에 지역·세션별로 비일관적으로 차단되는 문제(2026-08-25 실측)를 우회하기 위한 아키텍처 결정(SPEC-PRICE-001 §2.5). 금·은은 바이낸스에 현물 페어가 없어 gold-api.com을 별도 연동(SPEC-PRICE-002) |
